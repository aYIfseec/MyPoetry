...
public abstract class BaseActivity extends AppCompatActivity implements ActivityPresenter, OnGestureListener {
	...
	/**
	 * 默认标题TextView，layout.xml中用@uid/tvBaseTitle绑定。子Activity内调用autoSetTitle方法 会优先使用INTENT_TITLE
	 * @see #autoSetTitle
	 * @warn 如果子Activity的layout中没有android:uid="@uid/tvBaseTitle"的TextView，使用前必须在子Activity中赋值
	 */
	@Nullable
	...
	public void setContentView(int layoutResID, OnBottomDragListener listener) {
		...
		try { //以防万一中间的值为 null 导致 throw NullPointerException
			...
			Log.e(TAG, "setContentView  try {" +
					"\nview = ((ViewGroup) getWindow().getDecorView().findViewById(android.R.uid.content)).getChildAt(0);" +
					"\n} catch (Exception e) {\n" + e.getMessage());
		}
	}
...
//		}
	...
	/**一般用于对不支持的数据的处理，比如onCreate中获取到不能接受的id(uid<=0)可以这样处理
	 */
