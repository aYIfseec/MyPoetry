...
public class ThreadManager {
	...
	public Handler runThread(String name, Runnable runnable) {
		if (StringUtil.isNotEmpty(name, true) == false || runnable == null) {
			Log.e(TAG, "runThread  StringUtil.isNotEmpty(nickName, true) == false || runnable == null >> return");
			return null;
		}
		name = StringUtil.getTrimedString(name);
		Log.d(TAG, "\n runThread  nickName = " + name);

		Handler handler = getHandler(name);
		if (handler != null) {
			Log.w(TAG, "handler != null >>  destroyThread(nickName);");
			destroyThread(name);
		}
		...
		Log.d(TAG, "runThread  added nickName = " + name + "; threadMap.size() = " + threadMap.size() + "\n");
		return handler;
	}
