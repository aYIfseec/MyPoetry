...
, zuo.biao.library.interfaces.OnHttpResponseListener {
	...
	public void onHttpResponse(int requestCode, String resultJson, Exception e) {
		...
		try {
			...
			resultCode = jsonObject.getInt("resCode");
			resMsg = jsonObject.getString("resMsg");
			resultData = jsonObject.getString("resData");
		} catch (Exception e1) {
			Log.e(TAG, "onHttpResponse  try { sonObject = new JSONObject(resultJson);... >>" +
					" } catch (JSONException e1) {\n" + e1.getMessage());
			exception = e1;
		}
		Log.i(TAG, "onHttpResponse  resultCode = " + resultCode + "; resMsg = " + resMsg + "; resultData = " + resultData);

		if (listener == null) {
			listener = this;
		}
		if ((e == null && exception == null) || resultCode > 0 || StringUtil.isNotEmpty(resultData, true)) {
			listener.onHttpSuccess(requestCode, resultCode, resMsg, resultData);
		} else {
